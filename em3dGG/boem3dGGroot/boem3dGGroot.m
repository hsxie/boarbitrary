...
% for arbitrary distribution version with GPDF-GPDF expansion
...
for s=1:S
    if(ifv0(s)==1)
        ...
        lsmax(s)=floor((size(aslm{s},1)-1)/2); % 24-12-31 13:21
        % lsmax(s)=min(size(aslm{s},1)-1,18); % 24-12-29 15:47
        % msmax(s)=size(aslm{s},2)-1;
        msmax(s)=floor((size(aslm{s},2)-1)/2);
    end
end
...
for jk=1:nk
    ...
    if(mod(jk,1)==0)
        ...
        %     warning off;
        % options = optimset('Display','on','TolFun',1e-10,'Tolx',1e-10);
        ...
        %     'Tolx', 1e-12, 'FunctionTolerance', 1e-12);
        wx1=fsolve(@fDrGG,x01,options);
        % fDr=@(w) abs(fDrGH(w));
        % wx1=fminsearch(fDr,x01,options);
        tmp=wx1/wn
        wwx(jk,1)=wx1/wn;
    else
        wwx(jk,1)=NaN+1i*NaN;
...
if(1==0)
    close all;
    k=0.4;
    kx=k*kn*sin(theta); % set kperp
    kz=k*kn*cos(theta); % set kperp
    [xx,yy]=ndgrid((-0.2:0.2:1)*0.05,(-1:0.1:1)*0.05);
    zz=(xx+1i*yy)*wn;
    [nx,ny]=size(zz);
    fw=0.*zz;
    for jx=1:nx
        for jy=1:ny
            fw(jx,jy)=fDrGG(zz(jx,jy));
        end
    end
    %%
    figure;
    contour(xx,yy,real(fw),[0,0]*max(max(abs(real(fw)))),'-');hold on;colorbar;
    contour(xx,yy,imag(fw),[0,0]*max(max(abs(imag(fw)))),'--');hold on;colorbar;
    title(['k=',num2str(k)]);
%%
end
