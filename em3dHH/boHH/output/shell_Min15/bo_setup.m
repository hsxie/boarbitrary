...
savepath='../output/shell_Min15/'; % choose where to save the figures and outputs
iem=1; % only =1, electromagnetic run; 

if(iem~=3) % only needed for kinetic version
    N=10; % Number harmonics, enlarge N to make sure the results are convergent
    J=24; % J-pole, usually J=8 is sufficient; other choice: 4, 12, 16, 24
...
B0=32.8E-9; % magnetic field (Tesla)
% -1      5.447e-4     1.e6        5.0e0      5.0e0     0.0       0.0       1

% initialize the parameters k, theta, kz, kx
par=zeros(6,1);
par(1)=0.5; % k, =sqrt(kx^2+kz^2), normalized by kn
par(2)=89.0; % theta, the angle between k and B0, normalized by *pi/180
...
ilogb=0;
pa1=1.0; pa2=12.0; dpa=0.1; % 1st parameter a, depends on ipa
