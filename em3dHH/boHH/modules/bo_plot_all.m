...
h=figure('unit','normalized','Position',[0.01 0.45 0.75 0.4],...
...
%     semilogx(10.^papp,real(www),'g+','LineWidth',2); hold on;
    indr=find(real(www)>0);
    loglog(10.^papp(indr),real(www(indr)),'g.','LineWidth',2); hold on;
    xlim([min(10.^pa),max(10.^pa)]);
    ylim([1e3,1e13]);grid on;
  end  
...
if(1==1)
    ...
    save([savepath,'bo_',figstr,'_all.mat'],'epsilon0','c2','mu0','kB','qe',...
        'mp','me','ms','qs','ns0','Tzs0','Tps0','rhocs','par','iloga','ilogb',...
        'B0','J','N','iem','S','wcs','wps','ipa','ipb','pa','papp',...
        'ww','www','wn','kn');
end
