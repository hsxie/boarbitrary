% Hua-sheng XIE, huashengxie@gmail.com, ENN, 2024-12-31 08:17
% Hot magnetized plasma dispersion relation, for arbitrary distribution
% with GPDF for para & Hermite expansion for perp directions
% To update: 1.fsolve work not well for damped mode; 2. integral write out
% from fDrGH() to speed up

function f=fDrGH(w) % DR
...
for s=1:S % species
    ...
    for n=Nsss % sum_N
        ...
        if(ifv0(s)==1) % Hermite-Hermite lowest order, drift bi-Maxwellian ring beam
        ...
        else % 24-12-31 08:34 GPDF-Hermite

            Ans=zeros(msmax(s)+4,2); Bns=0.*Ans; Cns=0.*Ans;
            for m=0:(msmax(s)+2)
                % 24-12-21 07:52, calculate An, Bn, Cn using numerical integral
                idm=m+2; % to also store m-1 with m=0
                Ans(idm,1)=1.0/As(s)*funAn(n,as(s),ds(s),m,0); % p=0
                Ans(idm,2)=1.0/As(s)*funAn(n,as(s),ds(s),m,1); % p=1
                Bns(idm,1)=1.0/As(s)*funBn(n,as(s),ds(s),m,1); % p=1
                Bns(idm,2)=1.0/As(s)*funBn(n,as(s),ds(s),m,2); % p=2
                Cns(idm,1)=1.0/As(s)*funCn(n,as(s),ds(s),m,2); % p=2
                Cns(idm,2)=1.0/As(s)*funCn(n,as(s),ds(s),m,3); % p=3
            end
            ...
            for l=-lsmax(s):lsmax(s)
                for m=0:msmax(s)
                    Xsn_xx=Xsn_xx+aslm{s}(l-lmin+1,m+1)*((n*wcs(s)/(kz*vtzs(s))*Zlpns(l-lmin+2,1)+ ...
                        (l==0))*(-4*Ans(m+1+2,1)+2*m*Ans(m-1+2,1))+ ...
                        1i*Ans(m+2,2)*(vtps(s)/vtzs(s))^2*((l-1)*Zlpns(l-1-lmin+2,1)+ ...
                        2*l*Zlpns(l-lmin+2,1)+(l+1)*Zlpns(l+1-lmin+2,1)));
                    Xsn_xy=Xsn_xy+aslm{s}(l-lmin+1,m+1)*((n*wcs(s)/(kz*vtzs(s))*Zlpns(l-lmin+2,1) ...
                        )*(-4*Bns(m+1+2,1)+2*m*Bns(m-1+2,1))+ ...
                        1i*Bns(m+2,2)*(vtps(s)/vtzs(s))^2*((l-1)*Zlpns(l-1-lmin+2,1)+ ...
                        2*l*Zlpns(l-lmin+2,1)+(l+1)*Zlpns(l+1-lmin+2,1)));
                    Xsn_yy=Xsn_yy+aslm{s}(l-lmin+1,m+1)*((n*wcs(s)/(kz*vtzs(s))*Zlpns(l-lmin+2,1)+ ...
                        (l==0))*(-4*Cns(m+1+2,1)+2*m*Cns(m-1+2,1))+ ...
                        1i*Cns(m+2,2)*(vtps(s)/vtzs(s))^2*((l-1)*Zlpns(l-1-lmin+2,1)+ ...
                        2*l*Zlpns(l-lmin+2,1)+(l+1)*Zlpns(l+1-lmin+2,1)));
                    Xsn_xz=Xsn_xz+aslm{s}(l-lmin+1,m+1)*((n*wcs(s)/(kz*vtps(s))*Zlpns(l-lmin+2,2) ...
                        )*(-4*Ans(m+1+2,1)+2*m*Ans(m-1+2,1))+ ...
                        1i*Ans(m+2,2)*(vtps(s)/vtzs(s))*((l-1)*Zlpns(l-1-lmin+2,2)+ ...
                        2*l*Zlpns(l-lmin+2,2)+(l+1)*Zlpns(l+1-lmin+2,2)));
                    Xsn_yz=Xsn_yz+aslm{s}(l-lmin+1,m+1)*((n*wcs(s)/(kz*vtps(s))*Zlpns(l-lmin+2,2) ...
                        )*(-4*Bns(m+1+2,1)+2*m*Bns(m-1+2,1))+ ...
                        1i*Bns(m+2,2)*(vtps(s)/vtzs(s))*((l-1)*Zlpns(l-1-lmin+2,2)+ ...
                        2*l*Zlpns(l-lmin+2,2)+(l+1)*Zlpns(l+1-lmin+2,2)));
                    Xsn_zz=Xsn_zz+aslm{s}(l-lmin+1,m+1)*((n*wcs(s)/(kz*vtzs(s))*Zlpns(l-lmin+2,3) ...
                        )*(-4*Ans(m+1+2,1)+2*m*Ans(m-1+2,1))+ ...
                        1i*Ans(m+2,2)*(vtps(s)/vtzs(s))^2*((l-1)*Zlpns(l-1-lmin+2,3)+ ...
